...
export default {
  ...
  async [types.REMOVE_FINISH_NOTES]({
    dispatch,
    commit,
    state
  }) {
  ...
  async [types.SAVE_NOTES]({
    commit
  }, notes) {
    await storage.set('notes', notes)
  },
  async [types.UPLOAD_NOTES]({
    commit,
    state,
    dispatch
  }, notes) {
